...
contract BaseLogoNFT is ERC721, Ownable {
    ...
    constructor(
        address baseColorsAddress
    ) ERC721("TestLogoNFT", "TLNFT") Ownable(msg.sender) {
    ...
    function mint(uint256 baseColorTokenId) external payable {
        ...
        require(
            !_baseColorTokenIdUsed[baseColorTokenId],
            "Base color already used"
        );
        require(
            isTokenIdEligible(baseColorTokenId),
            "TokenId not eligible for minting"
        );
        ...
        string memory baseColorHex = baseColors.tokenIdToColor(
            baseColorTokenId
        );
        require(bytes(baseColorHex).length > 0, "Base color does not exist");

        // Get the base color name
        string memory baseColorName = baseColors.tokenAttributes(
            baseColorTokenId,
            "Color Name"
        );
    ...
    function batchMint(uint256[] calldata baseColorTokenIds) external payable {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            // Split payment
            uint256 splitAmount = MINT_PRICE / 2;
            (bool success1, ) = payable(baseColorOwner).call{
                value: splitAmount
            }("");
            require(success1, "Payment to base color owner failed");
            (bool success2, ) = payable(owner()).call{value: splitAmount}("");
